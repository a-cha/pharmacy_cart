...
@Component
public class MatcherReceiptItem implements MatcherStrategy {
	@Override
	...
	public void matchNoAccount() {
		throw new ModelException(ErrorCodes.NO_USER_NO_RECEIPT.name());
	}

	@Override
	public void matchUser(BaseItem item, UserAccountEntity user) {
		if (!user.getReceiptItems().contains((ReceiptItemEntity) item))
			throw new ModelException(ErrorCodes.NO_RECEIPT.name());
	}
